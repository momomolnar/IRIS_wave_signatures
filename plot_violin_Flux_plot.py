...
Make a violin plot for the inferred fluxes 
for the different solar distribution of v_rms
...
TT = np.load("/Users/molnarad/CU_Boulder/Work/Chromospheric_business/Comps_2/comps_code/T.npz")
T_Ca = TT["arr_0"]
T_freq = TT["arr_1"]
...
T_interp = np.interp(freq[int(freq_lim[0]):int(freq_lim[1])], T_freq, T_Ca) 

d = {"Region":[], "Acoustic Flux [W/m2]":[], "Spectral Line":[]}
        ...
        for jj in range(0, 250):
            if mask[ii*4, jj*4]!=0:
                rms_vel = np.sum(fft_data[int(freq_lim[0]):int(freq_lim[1]),
                                          ii*4, jj*4] / T_interp **2)*freq[1] * cs * rho_CaII * 1e6 
                print(rms_vel)
                if (rms_vel > 0) and (np.isnan(rms_vel) != True):
                    d = {"Region":mask_name, "Acoustic Flux [W/m2]":np.log10(rms_vel), 
                         "Spectral Line":"CaII IR"}
...
cs = 6e3
rho_MnI = 2e-7
aa = np.load("T_coeff_MnI.npz")
freq_range_T = aa["freq_range"] / 1e3
T_MnI = aa["T_coeff"]
...
for el1 in range(num_files):
    ...
    T_interp = np.interp(freq[int(freq_lim[0]):int(freq_lim[1])], freq_range_T, 
                         T_MnI)
    
    for el2 in range(Pxx[50:-50, :].shape[0]):
        rms_vel = np.sum(Pxx_denoise[el2+50, int(freq_lim[0]):int(freq_lim[1])]
                         / T_interp**2) * freq[1] * cs * rho_MnI * 1e6
        print(rms_vel)
        #print(rms_vel)
        if (rms_vel > 0) and (np.isnan(rms_vel) != True):
            d =   {"Region":"QS", "Acoustic Flux [W/m2]":np.log10(rms_vel), 
                   "Spectral Line":"MnI 280.19 nm"}
            IBIS_data_frame = IBIS_data_frame.append(d, ignore_index=True )
...
for el1 in range(num_files):
    ...
    for el2 in range(Pxx_denoise[50:-50, :].shape[0]):
        rms_vel = (np.sum(Pxx_denoise[50+el2, int(freq_lim[0]):int(freq_lim[1])]) 
                   * freq[1] * cs * rho_MgII * 1e6)
        print(rms_vel)
        if (rms_vel > 0) and (np.isnan(rms_vel) != True):
            d =   {"Region":"QS", "Acoustic Flux [W/m2]":np.log10(rms_vel), 
                   "Spectral Line":"MgII k3"}
            IBIS_data_frame = IBIS_data_frame.append(d, ignore_index=True)
...
for el1 in range(num_files):
    ...
    for el2 in range(Pxx[100:700, :].shape[0]):
        ...
        rms_vel = (np.sum(Pxx_denoise[el2+100, int(freq_lim[0]):int(freq_lim[1])]/(T_interp**2)) 
                   * freq[1] * cs * rho_MnI * 1e6)
        print(rms_vel)
        #print(rms_vel)
        if (rms_vel > 0) and (np.isnan(rms_vel) != True):
            d =   {"Region":"Plage", "Acoustic Flux [W/m2]":np.log10(rms_vel), 
                   "Spectral Line":"MnI 280.19 nm"}
            IBIS_data_frame = IBIS_data_frame.append(d, ignore_index=True)
    #print(f"V_RMS_{el1} is {v_rms[el1]} $\pm$ {Pxx_noise_ave[el1]}.") 
...
# plot
ax = sns.violinplot(x=IBIS_data_frame["Region"], y=IBIS_data_frame["Acoustic Flux [W/m2]"], 
                   hue=IBIS_data_frame["Spectral Line"],
                   hue_order=["MnI 280.19 nm", "CaII IR", "MgII k3"])
ax.set(ylabel=("Log10[Acoustic Flux [W.m$^{-2}$]]"))
ax.set(ylim=(-0, 5))
